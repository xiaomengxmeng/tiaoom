<template>
  <section class="flex flex-col items-center justify-center p-2 py-4" ref="containerRef">
    <!-- 棋盘 -->
    <div class="inline-block p-2 m-auto border rounded shadow-2xl bg-base-300 border-base-content/20 select-none relative">
    ...
    <div v-if="isPlaying" class="flex items-center justify-center gap-3 mt-2 -mb-2 text-sm">
      ...
      <b class="text-base-content">{{ currentPlayer?.name }}</b>      <span class="text-xs bg-base-content/10 px-2 py-0.5 rounded" :class="{ 'text-error': timer < 10 }">
        {{ timerStr }}
      </span>    </div>
